...
#		employee[empID] = empName ":" empPosi;
			...
			if(saID in sales)
				sales[saID] = sales[saID] + procost[salesID] * salesqnt;
			else 
...
#
					position = a[2];
				#	finaldata[1] = -split(empName,a,"-");

#	
					printf ("%-20s %-15s % 15.2f\n", empName, position, sort_sales[i]);
				#	printf ("%-20s %-15s %15.2f\n", finaldata[1], finaldata[2], sort_sales[i]);
